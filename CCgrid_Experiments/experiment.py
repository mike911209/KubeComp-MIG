...
for subfolder in subfolders:
    ...
    for idx, file_path in enumerate(file_paths):
        ...
        # 移除重複的列，只保留時間最早的那一行
        df = df.sort_values(by='Time').drop_duplicates(subset=df.columns[1], keep='first')
        ...
        plt.gcf().set_size_inches(11, 3.8)
        plt.plot(df['Time'], df.iloc[:, 1], label=mig_slice, color=colors(idx), linewidth=4)
       

        
    # 讀取 SLO.json 檔案
    # slo_file_path = os.path.join('SLO.json')
    # if os.path.exists(slo_file_path):
    #     with open(slo_file_path, 'r') as slo_file:
    #         slo_data = json.load(slo_file)
    #         slo_upper_bound = slo_data.get('SLO_upper_bound', None)
    #         slo_lower_bound = slo_data.get('SLO_lower_bound', None)
            
    #         # 繪製水平線
    #         if slo_upper_bound is not None:
    #             plt.axhline(y=slo_upper_bound, color='grey', linestyle='--', label='Scaling Up')
    #         if slo_lower_bound is not None:
    #             plt.axhline(y=slo_lower_bound, color='grey', linestyle='--', label='Scaling Down')
      

    # 設定圖表標題和標籤
    title_file_path = os.path.join(parent_folder, subfolder, 'title.txt')
