...
class RequestSender:
	...
	def constant_rate(self, total_time, intervals=3):
		start_time = time.time()
		request_count = 0
		while time.time() - start_time < total_time:
			if not isinstance(intervals, list):
				intervals = [(intervals, total_time)]
			for interval, period in intervals:
				start_period_time = time.time()
				while start_period_time + period <= time.time():
	...
	def linear_rate(self, total_time, min_interval, slope):
		...
		while time.time() - start_time < total_time:
			...
			if not self.send_request(model):
				break
			request_count += 1
			interval = min_interval + slope * time.time()
...
def main():
	...
	parser.add_argument("rate", type=str, choices=["const", "osc", "hybrid", "rand", "linear"], help="""
					 Type of rate to use : 
